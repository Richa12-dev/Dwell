...
//
// Exports:
//   useSignedImageUrls(rawImages, token)  — hook for property image arrays
...
export const extractS3Key = (raw) => {
  ...
  if (typeof raw === 'string') {
    ...
    // Already a signed URL — use directly, return null so caller skips signing
    if (s.includes('X-Amz-Algorithm')) return null;

    // Full S3 URL — extract everything after the bucket host
...
// ─────────────────────────────────────────────────────────────
// useSignedProfileImage — hook for a single profile image
//
// Mirrors useSignedImageUrls but for one profile photo.
// Fetches the raw image key directly from the API (bypasses
// Redux stale state), builds the correct S3 key, signs it,
// and returns a ready-to-use presigned URL.
//
...
//
// deps — extra values that trigger a re-fetch (e.g. uploadVersion counter)
// ─────────────────────────────────────────────────────────────
export const useSignedProfileImage = (token, deps = []) => {
  ...
  useEffect(() => {
    ...
    const resolve = async () => {
      ...
      try {
        // Step 1: fetch raw profileImage value directly from backend
        // fetchProfileImageKey bypasses Redux Persist stale state
        ...
        // so we pass just the filename for new records or extract it from
        // full URLs for old records.
        //
        // New record in DB: "UUID.jpg"
        //   → key = "UUID.jpg"
        //   → backend signs as "uploads/UUID.jpg" ✅
        //
        // Old record in DB: "profiles/UUID.jpg" or full https://...
        //   → extract filename → "UUID.jpg"
        //   → backend signs as "uploads/UUID.jpg" ✅
        let s3Key;
        ...
        const signedUrl = await fetchSignedUrl(s3Key, token);
  ...
  // Re-run when token changes or when caller increments uploadVersion
