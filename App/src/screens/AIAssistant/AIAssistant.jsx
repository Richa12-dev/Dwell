...
import { updatePropertyLocally } from '../../Redux/Properties/propertiesSlice';
      ...
      <Hyperlink
        ...
        <Text
          style={[
            styles.messageText,
            {
              color: isUser ? Colors.black : (isError ? '#d32f2f' : Colors.black),
            },
          ]}>
          {messageContent}
        </Text>
...
const AIAssistant = ({ navigation, route }) => {
  ...
  // ✅ Grab landlordId so we can refresh properties after AI updates them
  const landlordId = rawLoginState?.landlordId ||
    rawLoginState?.userData?.landlordId ||
    rawLoginState?.user?.landlordId || null;
  ...
  // ✅ Handle navigation params and reset suggestions on screen focus
  useFocusEffect(
    useCallback(() => {
      const shouldHideSuggestions = route?.params?.hideSuggestions || false;
      
      console.log('🔍 AIAssistant focused - hideSuggestions:', shouldHideSuggestions);
      console.log('📊 Current messages count:', messages.length);
      
      // Only update suggestions if there are no messages yet
      if (messages.length === 0) {
        setShowSuggestions(!shouldHideSuggestions);
        
        // Load suggestions only if they should be shown
      ...
      return () => {
        // Cleanup if needed
        console.log('🔚 AIAssistant unfocused');
      };
  ...
  // ✅ Refresh landlord properties in Redux after AI modifies a property.
  // Detects common AI response phrases that indicate a property was updated.
  const refreshPropertiesIfUpdated = useCallback((aiResponse) => {
    if (!aiResponse || !landlordId) return;

    const lowerResponse = aiResponse.toLowerCase();
    const propertyUpdateKeywords = [
      'updated', 'changed', 'modified', 'set to', 'availability',
      'rent', 'monthly_rent', 'price', 'property has been', 'successfully updated',
    ];
    const isPropertyUpdate = propertyUpdateKeywords.some(kw => lowerResponse.includes(kw));

    if (isPropertyUpdate) {
      const availableToken = getAvailableToken();
      if (availableToken) {
        console.log('🔄 AI updated a property — refreshing landlord properties in Redux...');
        dispatch(getLandlordProperties({ landlordId, token: availableToken }));
      }
    }
  ...
  const handleSendMessage = useCallback(async (messageOverride = null) => {
    ...
    try {
      ...
      if (currentImage) {
        ...
        console.log('📤 Sending text message to OLD API...');
      ...
      if (result.response && isMountedRef.current) {
        // ✅ If the AI updated a property, sync Redux immediately so
        // LandlordProperties reflects the change without a logout/login.
  ...
  }, [messageText, selectedImage, isLogged, currentSessionId, loading, getAvailableToken, dispatch, navigation, loadSuggestions, refreshPropertiesIfUpdated]);
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            // Reset suggestions based on navigation params
  ...
  // ✅ Render suggestions - simplified logic
